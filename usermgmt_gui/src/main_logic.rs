...
//! Global state can be change directly by either
//!
//! - GUI elements like a button
//! - application of returned results from an IO background task.
//!
...
//! An IO background task is what accomplishes the management of user LDAP and Slurm in the cluster.

/// Go here If you want to add a new selectable view.
mod current_selected_view;
/// Queries for pending Io background task.
/// Go here if you want to put some work into a background Os thread without blocking GUI.
mod query_io_tasks;
/// Go here if you want to add a new value which influences the look of the GUI and contained the
/// settings files.  
mod settings;
